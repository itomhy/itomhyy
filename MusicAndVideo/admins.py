...
async def skip(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        OP = "**🗑️ Aşağıdakı mahnılar növbədən silindi: -**"
        if chat_id in QUEUE:
...
@Client.on_message(filters.command(["end", "dayan"], prefixes=f"{HNDLR}"))
@authorized_users_only
...
async def pause(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                f"**⏸ Pauza edildi.**\n\n• Davam etmək üçün » **{HNDLR}resume** basın"
            )
        except Exception as e:
            await m.reply(f"**Xəta** \n`{e}`")
    else:
...
async def resume(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                f"**▶ Davam etdirilir**\n\n• Pauza etmək üçün » **{HNDLR}pause** basın"
            )
        except Exception as e:
            await m.reply(f"**Xəta** \n`{e}`")
    else:
