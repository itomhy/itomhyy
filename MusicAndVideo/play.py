...
AMBILFOTO = [
    "https://telegra.ph/file/04c7d5d5079a585491aff.jpg",
    "https://telegra.ph/file/04c7d5d5079a585491aff.jpg",
]
    ...
    if replied:
        if replied.audio or replied.voice:
            ...
            if chat_id in QUEUE:
                ...
                await m.reply_photo(
                    photo="https://telegra.ph/file/04c7d5d5079a585491aff.jpg",
                    caption=f"""
...
""",
        ...
        if len(m.command) < 2:
            ...
            if search == 0:
                await huehue.edit("**İstədiyiniz mahnını tapa bilmədim🥲**")
            else:
                songname = search[0]
                url = search[1]
                hm, ytlink = await ytdl(url)
                if hm == 0:
                    await huehue.edit(f"**YTDL Xətası ⚠️** \n\n`{ytlink}`")
                else:
                    if chat_id in QUEUE:
...
""",
        ...
        if len(m.command) < 2:
            await m.reply(
                "**Adı yazın, və ya, hər hansı bir mahnı, video faylına yanıt verin**"
            )
        else:
            await m.delete()
            huehue = await m.reply("**🔎 Axtarılır...**")
...
@Client.on_message(filters.command(["kanal"], prefixes=f"{HNDLR}"))
...
""",
            ...
            await hmm.delete()
            await m.reply(
                f"➕ Növbəyə {lmt} mahnı əlavə edilir\n• Mahnı siyahsına baxmaq üçün **{HNDLR}playlist** basın"
            )
        except Exception as e:
            await hmm.edit(f"**Xəta** \n`{e}`")


@Client.on_message(filters.command(["playlist", "siyahi"], prefixes=f"{HNDLR}"))
